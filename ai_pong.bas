...
WHILE NOT WINDOWSHOULDCLOSE()
    ...
    REM Predict where ball will be when it reaches the right side
    IF ballSpeedX > 0 THEN
        REM Ball moving towards AI paddle
        VAR timeToReach = (750 - ballX) / ballSpeedX
        VAR predictedY = ballY + (ballSpeedY * timeToReach)
        
        REM Keep prediction within bounds
        WHILE predictedY < 0 OR predictedY > 600 - BALL_SIZE
            IF predictedY < 0 THEN predictedY = -predictedY
            IF predictedY > 600 - BALL_SIZE THEN predictedY = 2 * (600 - BALL_SIZE) - predictedY
        WEND
        
        REM Move AI paddle towards predicted position
        VAR paddleCenter = rightPaddleY + PADDLE_HEIGHT / 2
        VAR targetY = predictedY - PADDLE_HEIGHT / 2 + AI_PREDICTION_OFFSET
        
        IF paddleCenter < targetY THEN
            rightPaddleY = rightPaddleY + AI_PADDLE_SPEED
        ELSEIF paddleCenter > targetY THEN
            rightPaddleY = rightPaddleY - AI_PADDLE_SPEED
        ENDIF
    ELSE
        REM Ball moving away, center the paddle
        VAR paddleCenter = rightPaddleY + PADDLE_HEIGHT / 2
        VAR screenCenter = 300
        IF paddleCenter < screenCenter THEN
            rightPaddleY = rightPaddleY + AI_PADDLE_SPEED / 2
        ELSEIF paddleCenter > screenCenter THEN
            rightPaddleY = rightPaddleY - AI_PADDLE_SPEED / 2
        ENDIF
    ENDIF
