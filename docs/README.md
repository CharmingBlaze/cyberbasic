# CyberBasic Documentation

Welcome to the CyberBasic documentation! This directory contains all the documentation for the CyberBasic programming language.

## 📚 Documentation Structure

### User Guides (Main Documentation)
- **[Documentation Index](DOCUMENTATION_INDEX.md)** - Start here! Complete guide to all documentation
- **[Getting Started Guide](GETTING_STARTED.md)** - Installation and first steps
- **[Quick Reference](QUICK_REFERENCE.md)** - Quick syntax reference
- **[BASIC Programming Guide](BASIC_PROGRAMMING_GUIDE.md)** - Complete language reference

### Game Development
- **[Game Development Guide](GAME_DEVELOPMENT_GUIDE.md)** - Complete guide to making games
- **[2D Graphics API Guide](2D_GRAPHICS_GUIDE.md)** - 2D rendering, sprites, textures
- **[3D Graphics API Guide](3D_GRAPHICS_GUIDE.md)** - 3D rendering, models, cameras, lighting
- **[Distribution Guide](DISTRIBUTION_GUIDE.md)** - How to distribute your games

### Advanced Features
- **[Advanced Features Guide](ADVANCED_FEATURES_GUIDE.md)** - Enums, tuples, coroutines, ECS, state machines
- **[ECS System Guide](ECS_SYSTEM_GUIDE.md)** - Entity-Component-System architecture
- **[Modern State System Guide](MODERN_STATE_SYSTEM_GUIDE.md)** - State machine system

### Additional Resources
- **[GUI System Guide](GUI_SYSTEM_GUIDE.md)** - GUI system documentation
- **[Contributing Guide](CONTRIBUTING.md)** - How to contribute
- **[Coding Style](CODING_STYLE.md)** - Code style guidelines

### Implementation Documentation
Implementation notes and status documents are in the [`implementation/`](implementation/) subdirectory.

---

**Start with the [Documentation Index](DOCUMENTATION_INDEX.md) for a complete overview!**



