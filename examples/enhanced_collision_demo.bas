REM Enhanced Collision Demo with New Features
REM Showcases: MOD operator, BREAK statement, DRAWTRIANGLE, enhanced mouse input
PRINT "Enhanced Collision Demo Starting..."

INITWINDOW(800, 600, "Enhanced Collision Demo")
SETTARGETFPS(60)
...
WHILE running
  ...
  REM Handle input
  IF ISKEYPRESSED(87) THEN LET player_y = player_y - player_speed ENDIF  REM W
  IF ISKEYPRESSED(83) THEN LET player_y = player_y + player_speed ENDIF  REM S
  IF ISKEYPRESSED(65) THEN LET player_x = player_x - player_speed ENDIF  REM A
  IF ISKEYPRESSED(68) THEN LET player_x = player_x + player_speed ENDIF  REM D
  ...
  IF player_y > 600 - player_h THEN LET player_y = 600 - player_h ENDIF
  
  REM Check collisions using MOD for periodic effects
  ...
  REM Enhanced mouse input using new functions
  ...
  REM Calculate distances with complex expressions
  ...
  REM Use MOD operator for periodic effects
  IF game_time MOD 60 = 0 THEN
    REM Every 60 frames, add a small bonus
    LET score = score + 1
  ENDIF
  ...
  REM Draw obstacle (red, with triangle indicator)
  ...
  REM Add triangle indicator on obstacle
  DRAWTRIANGLE(obstacle_x + obstacle_w/2, obstacle_y - 10, obstacle_x + 10, obstacle_y + obstacle_h + 10, obstacle_x + obstacle_w - 10, obstacle_y + obstacle_h + 10, 255, 255, 0)
  
  REM Draw target (green, with triangle indicator)
  LET target_color = 0
  ...
  REM Add triangle indicator on target
  DRAWTRIANGLE(target_x + target_w/2, target_y - 8, target_x + 8, target_y + target_h + 8, target_x + target_w - 8, target_y + target_h + 8, 255, 255, 255)
  
  REM Draw player (blue, changes color on collision, with triangle)
  LET player_color = 0
  ...
  REM Add triangle indicator on player
  DRAWTRIANGLE(player_x + player_w/2, player_y - 8, player_x + 8, player_y + player_h + 8, player_x + player_w - 8, player_y + player_h + 8, color_cycle, 255, 255)
  ...
  REM Draw UI
  DRAWTEXT("Enhanced Collision Demo", 10, 10, 24, 255, 255, 255)
...
PRINT "Enhanced collision demo completed!"
PRINT "Final Score: " + STR(score)
PRINT "Game Time: " + STR(game_time) + " frames"
PRINT "Features demonstrated:"
PRINT "  - MOD operator for periodic effects"
PRINT "  - Enhanced mouse input (GETMOUSEX/GETMOUSEY)"
PRINT "  - DRAWTRIANGLE for visual indicators"
PRINT "  - Complex expressions in function calls"
PRINT "  - Improved collision detection"
